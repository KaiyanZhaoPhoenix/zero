...
class EnhancedDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        for col in self.feature_columns + [self.target_column]:
            if col in mask_cols:
                ...
                std = train_df[col].std() if max_val > min_val else 1.0
                range_val = max_val - min_val if max_val > min_val else 1.0

                self.scalers[col] = {'min': min_val, 'max': max_val, 'mean': mean_val , 'std': std, 'range': range_val, "kind": "minmax"}
    ...
    def _normalize(self, df, val=False):
        ...
        for col in self.feature_columns:
            ...
            if sc.get("kind") == "mask":
                ...
                # X[col] = (df[col] - sc['mean']) / sc['std']
                # X[col] = 2*(df[col] - sc['min']) / sc['range'] -1
                # X[col] = (df[col] - sc['min']) / sc['range'] -0.5
                # X[col] = (df[col] - sc['min']) / sc['range'] + 1
                X[col] = (df[col] - sc['mean']) / sc['range']
        # y = (df[self.target_column] - self.scalers[self.target_column]['min']) / self.scalers[self.target_column]['range']
        # y = (df[self.target_column] - self.scalers[self.target_column]['mean']) / self.scalers[self.target_column]['std']
        # y = 2*(df[self.target_column] - self.scalers[self.target_column]['min']) / self.scalers[self.target_column]['range'] - 1
        # y = (df[self.target_column] - self.scalers[self.target_column]['min']) / self.scalers[self.target_column]['range'] - 0.5
        # y = (df[self.target_column] - self.scalers[self.target_column]['min']) / self.scalers[self.target_column]['range'] + 1
        y = (df[self.target_column] - self.scalers[self.target_column]['mean']) / self.scalers[self.target_column]['range']
        return torch.tensor(X.values, dtype=torch.float32), torch.tensor(y.values, dtype=torch.float32).view(-1, 1)

    def train_dataloader(self):
        # sys.exit(f"Anon: {len(self.train_dataset)}")
        # sys.exit(f"Anon: {len(DataLoader(self.train_dataset, batch_size=self.batch_size, shuffle=True, num_workers=self.num_workers))}")
...
class PhysZeroMLP(pl.LightningModule):
    def __init__(self, input_size, feature_map, hidden_size, depth, lr, lambda_constraint=1.0):
        ...
        # self.network.append(fc_module((hidden_size[-1] + addtemp, 1), activation='relu', dropout_rate=0))
    ...
    def training_step(self, batch, _):
        ...
        self.log_dict({'train/lr': self.optimizer.param_groups[0]['lr'],
    ...
    def validation_step(self, batch, _):
        ...
        if loss<lowest_loss:
            lowest_loss=loss
            torch.save(self.state_dict(), "models/PhysZeroMLP_final_best.pth")

        self.log_dict({'val/lr': self.optimizer.param_groups[0]['lr'],
                       'val/total_loss': loss,
    ...
    def configure_optimizers(self):
        global max_epoch
        # self.optimizer = torch.optim.Adam(self.parameters(), lr=self.hparams.lr*1e1)
        self.optimizer = Anon(self.parameters(), lr=self.hparams.lr*1e1, gamma=1.3)
        self.scheduler=torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, eta_min=self.hparams.lr*1e-1,T_max=bs*max_epoch)
        # print("Anon: pass")
...
def evaluate(model, datamodule, device="cpu"):
    ...
    # y_true = y_true_norm * sc['std'] + sc['mean']
    # y_pred = y_pred_norm * sc['std'] + sc['mean']
    # y_true = (y_true_norm + 1) * sc['range']/2 + sc['min']
    # y_pred = (y_pred_norm + 1) * sc['range']/2 + sc['min']
    # y_true = (y_true_norm + 0.5) * sc['range'] + sc['min']
    # y_pred = (y_pred_norm + 0.5) * sc['range'] + sc['min']
    # y_true = (y_true_norm - 1) * sc['range'] + sc['min']
    # y_pred = (y_pred_norm - 1) * sc['range'] + sc['min']
    y_true = y_true_norm * sc['range'] + sc['mean']
    y_pred = y_pred_norm * sc['range'] + sc['mean']
    ...
    ax.plot(y_true, label='Actual Temperature', color='b', linewidth=2)
    ax.plot(y_pred, label='Predicted Temperature', color='r', linestyle='--', alpha=0.8)
