...
class Anon(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-16, gamma=0.,
                 weight_decay=0, weight_decouple=False, fixed_decay=False, rectify=False,
        ...
        print("lr={}, gamma={}, eps={}, betas={}, wd={}".format(lr, gamma, eps, betas, weight_decay))
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if grad.is_sparse:
                    raise RuntimeError(
                        'Anon does not support sparse gradients'
                    )
                ...
                beta3=beta2
                # beta3=0.5
                if state['step'] == 2 ** t or beta3>0.9:
                    if t == 0:
                        state['t'] += 1
                        bias_correction2 = 1 - beta2
                        fixed_lr.add_(exp_avg_sq.div_(bias_correction2).add_(group['eps']).pow_(group['gamma']).rsqrt_())
                        exp_avg_sq.zero_()

                    else:
                        state['t'] += 1
                        bias_correction2 = 1 - beta2 ** (state['step']-state['ot'])
                        fixed_lr.pow_(2).reciprocal_().mul_(beta3).add_(exp_avg_sq.div_(bias_correction2).add_(group['eps']).pow_(group['gamma']).mul_(1-beta3)).rsqrt_()
                        exp_avg_sq.zero_()
                    state['ot'] = state['step']
